...
names = spark.read.schema(schema).option("sep", " ").csv("/Users/marshad/Desktop/SparkCourse/data/Marvel-names.txt")
...
* Anytime you'll perform more than one action on a dataframe, youshould cache it!
    * Otherwise, spark might re-evalute the entire datagrame all over again!
* `cache()` vs `persist()`
